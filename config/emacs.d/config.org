...
#+BEGIN_SRC emacs-lisp
  ...
  (defun diego/lecture-notes ()
    (interactive)
    (ivy-read "directory: " '("~/MIT 2016-2017/16.410/notes/"
                              "~/MIT 2016-2017/16.621/notes/"
                              "~/MIT 2016-2017/Ethics/notes/")
              :action (lambda (x)
                        (find-file
                         (concat x (format-time-string "%Y-%m-%d_lecture.org" ))))
...
#+BEGIN_SRC emacs-lisp
  (defun diego/config-evil-leader ()
    ...
    (evil-leader/set-key
      ...
      "fc"        'evil-avy-goto-char-2
      "fw"        'evil-avy-goto-word-1
      "g"         'magit-status
      "h"         'split-window-below
      "i"         'ivy-imenu-anywhere     ;; Jump to function in buffer
      "k"         'kill-this-buffer
      "la"        'diego/toggle-linum-fci 
      "lc"        'fci-mode
      "ld"        'delete-trailing-whitespace
      "lf"        'flycheck-mode
      "ll"        'linum-mode
    ...
    (define-key universal-argument-map (kbd ",0") 'universal-argument-more)
    ...
    (defun diego/toggle-linum-fci ()
      (interactive)
      (cond ((and (eq linum-mode t) (eq fci-mode t))
             (linum-mode -1)
             (fci-mode -1))
            ((and (eq linum-mode nil) (eq fci-mode nil))
             (linum-mode 1)
             (fci-mode 1)))))
#+END_SRC
*** Modes
...
#+BEGIN_SRC emacs-lisp
  (use-package doom-themes
    ...
    (custom-theme-set-faces
     ...
     '(org-level-4 ((t (:foreground "#8FA1B3"))))
