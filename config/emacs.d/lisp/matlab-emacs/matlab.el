...
If font lock is not loaded, lay in wait."
  (if (and (featurep 'custom) (fboundp 'custom-declare-variable))
	...
	(defface matlab-simulink-keyword-face
	  (list
	   (list t
		 (list :background (face-background font-lock-builtin-face)
    ...
    (cond ((facep 'font-lock-builtin-face)
	   (copy-face 'font-lock-builtin-face 'matlab-simulink-keyword-face))
	  (t
    ...
    (cond ((facep 'font-lock-builtin-face)
	   (copy-face 'font-lock-builtin-face 'matlab-nested-function-keyword-face))
	  (t
	   (make-face 'matlab-nested-function-keyword-face)))
    
    ;; Now make some cross-function variable faces
    (cond ((facep 'font-lock-builtin-face)
	   (copy-face 'font-lock-builtin-face 'matlab-cross-function-variable-face))
	  (t
