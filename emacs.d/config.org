...
* Contents                                                              :toc:
 ...
 - [[#functions][Functions]]
   ...
   - [[#window][Window]]
   - [[#editing][Editing]]
   - [[#org-mode-helpers][Org-mode helpers]]
   - [[#misc][Misc.]]
...
#+begin_src emacs-lisp
(load-file "~/.emacs.d/lisp/trouble-with-the-trees.el")
#+end_src
* Functions
Stolen and original functions and macros.
...
(defun download-file ()
  ...
  (let* ((file-url (read-from-minibuffer "URL: "))
         (file-name (read-from-minibuffer "File : " (concat default-directory
...
(defun diego/wiki ()
  (interactive)
  (let ((input (read-from-minibuffer "Wiki search: ")))
    (with-output-to-temp-buffer "*wiki*"
      (princ (shell-command-to-string (format "wiki -w 80 %s" input))))
...
(defun diego/evil-bindings ()
  ...
  (dolist (key '([mouse-1]
                 ...
                 [double-wheel-up]
                 [triple-wheel-up]))
    (define-key evil-motion-state-map key #'silence)))
...
(use-package bind-map
  ...
  (bind-map diego-default-map
    ;; :keys ("M-m")
    :evil-keys ((concat "C-" diego-evil-leader-key))
...
*** Copture config
#+begin_src emacs-lisp
(defun diego/org-capture-config ()
...
(use-package org
  ...
  (diego/org-bindings)
  (add-hook 'org-agenda-mode-hook #'diego/org-agenda-config)
  (add-hook 'org-capture-mode-hook #'diego/org-capture-config)
  (add-hook 'org-mode-hook #'diego/org-config)
  (require 'ox-extra)
