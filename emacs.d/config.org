...
(use-package company
  ...
  :config
  (defun company-mode/backend-with-yas-and-dabbrev (backend)
    "Source: http://tinyurl.com/n63vfhw"
    (if (and (listp backend) (member 'company-yasnippet backend))
        backend
      (append (if (consp backend) backend (list backend))
              '(:with company-yasnippet company-dabbrev))))
  (setq company-backends (mapcar #'company-mode/backend-with-yas-and-dabbrev
                                 company-backends))
