...
(defvar diego/mode-line
  '((:eval (with-evil-tag-color
    ...
    ;; Line/column number
    (:eval (with-window-status
             (propertize-active window-active? "%4l:%2c  " 'face '(:foreground "#A89984"))))
    ;; Major mode
    (:eval
     (with-window-status
       ...
       (when (and (bound-and-true-p flycheck-mode)
         ...
         (concat
          (cl-loop for state in '((error . "#FB4933")
                   ...
                   when lighter
                   concat (propertize-active window-active? lighter 'face `(:foreground ,(cdr state)))
                   )
          " "))))
    ;; Input method
    (:eval
