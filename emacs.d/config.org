...
(defvar diego/mode-line
  '((:eval (when  (featurep 'evil)
    ...
    (:propertize mode-name face (:foreground "#A89984"))
    " %4l:%2c "
    (:eval (when (or current-input-method
    ...
    " "
    (:eval (when (and (bound-and-true-p anzu--state))
             (anzu--update-mode-line)))
    (:eval nil ;; (if (and default-directory
           ;;        (file-remote-p default-directory 'host))
           ;;     (concat " @" (file-remote-p default-directory 'host)":"(file-remote-p default-directory 'user))
           ;;   (concat " @" (system-name)))
           )
    ;; mode-line-remote
    (:eval (when vc-mode
             (powerline-raw
              (concat " (" (propertize vc-mode 'face '(:foreground "#B8BB26")) "|"
                      (when (buffer-file-name)
                        (pcase (vc-state (buffer-file-name))
                          (`up-to-date (propertize "✓" 'face '(:foreground "#B8BB26")))
                          (`edited (propertize "✚" 'face '(:foreground "#FB4933")))
                          (`added (propertize "●" 'face '(:foreground "#83A598")))
                          (`unregistered "…")
                          (`removed "del")
                          (`needs-merge (propertize "✖" 'face '(:foreground "#FB4933")))
                          (`needs-update "upd")
                          (`ignored "ign")))
                      ")"))))
    " "))
...
(setq-default mode-line-format
              '((:eval (progn
                         (while (< (length (format-mode-line diego/mode-line))
                                   (window-total-width))
                           (setf (nth 8 diego/mode-line) (concat " " (nth 8 diego/mode-line))))
                         (while  (> (length (format-mode-line diego/mode-line))
                                    (window-total-width))
                           (setf (nth 8 diego/mode-line) (substring (nth 8 diego/mode-line) 0 -1)))
                         (format-mode-line diego/mode-line)))))
#+end_src
** Functions
*** Dotfiles
#+begin_src emacs-lisp
