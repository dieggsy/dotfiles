...
[[https://github.com/bbatsov/emacs-lisp-style-guide#source-code-layout--organization][emacs-lisp-style-guide]], and silence some byte-compile warnings. Now
technically, there's not really any point to byte-compiling your init.el, so
erm, don't follow my example there? Whatever.
...
#+begin_src emacs-lisp
(when (file-exists-p  "~/.emacs.d/lisp/there-is-trouble-with-the-trees.el")
  ...
  (defun diego/org-hugo-export ()
    ...
    (save-excursion
      ...
      (let* ((org-pandoc-format 'markdown)
        ...
        (if (member "draft" tags)
            (message "Post draft not exported")
          (progn
            ;; Make the export
            (org-export-to-file
                'pandoc
