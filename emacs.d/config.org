...
(use-package eshell-prompt-extras
  :after eshell
  :commands epe-colorize-with-face
  :config
  (autoload 'epe-theme-dakrone "eshell-prompt-extras")
  (setq eshell-highlight-prompt nil
        eshell-prompt-function 'epe-theme-diego
        epe-git-dirty-char "…")
  (require 'cl)
  (defface epe-ahead-behind-face '((t (:foreground "#D3869B")))
    "WHat")

  (defface epe-added-face '((t (:foreground "#83a598")))
    "Ahh")

  (defface epe-modified-face '((t (:foreground "#FB4933" )))
    "Things")

  (defun diego/epe-git-helper (command)
    (let ((output (shell-command-to-string command)))
      (if (not (string= output ""))
          (substring output 0 (1- (length output)))
        output)))

  (defun diego/epe-git-modified (symbol)
    (let ((result (diego/epe-git-helper
                   (concat epe-git-status
                           " | grep '^ M \\|^AM \\|^ T ' | wc -l"))))
      (if (not (string= result ""))
          (concat symbol result)
        result)))

  (defun diego/epe-git-added (symbol)
    (let ((result (diego/epe-git-helper
                   (concat epe-git-status
                           " | grep '^A \\|^M | wc -l'"))))
      (if (not (string= result ""))
  ...
  (defun diego/epe-git-ahead (symbol)
    (let ((result (diego/epe-git-helper
                   (concat epe-git-status
                           " | grep -oP 'ahead \\K\\d+'"))))
      (if (not (string= result ""))
  ...
  (defun diego/epe-git-behind (symbol)
    (let ((result (diego/epe-git-helper
                   (concat epe-git-status
                           " | grep -oP 'behind \\K\\d+'"))))
      (if (not (string= result ""))
    ...
    (concat
     ...
     (when (epe-git-p)
       (concat
        ...
        (epe-colorize-with-face
         (concat (diego/epe-git-ahead "↑")
                 (diego/epe-git-behind "↓"))
         'epe-ahead-behind-face)
        "|"
        (epe-colorize-with-face (diego/epe-git-added "●") 'epe-added-face)
        (epe-colorize-with-face (diego/epe-git-modified "✚") 'epe-modified-face)
        (epe-git-dirty)
        ;; (epe-git-untracked)
     ...
     (epe-colorize-with-face (fish-path (eshell/pwd) 0) 'epe-dir-face)
