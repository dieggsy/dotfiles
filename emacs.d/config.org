   ...
   - Based on xwl-erc-format-nick"
    (when user
      (let* ((nick (erc-server-user-nickname user))
             ...
             (nick-was-last (string= (gethash channel d/erc-last-speaker)
                                     nick)))
        (when (not (gethash channel d/erc-last-speaker))
          (puthash channel nick d/erc-last-speaker))
        (prog1
            (concat
             (format
              "%10.10s"
              (concat
               ...
               (cond (nick-was-last
                      (erc-propertize "↳" 'face (gethash nick erc-hl-nicks-face-table)))
                     ((> (length nick) 10)
                      (erc-propertize
                       nick 'face (erc-hl-nicks-make-face nick)))
                     (t nick))))
             (if (and (> (length nick) 10) (not nick-was-last))
                 (erc-propertize "…   " 'face '(:foreground "#D3869B"))
               "    "))
          (puthash channel nick d/erc-last-speaker)))))

  (defun d/erc-format-my-nick ()
