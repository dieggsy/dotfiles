...
#+begin_src emacs-lisp
(with-eval-after-load 'org
  (defun diego|create-block-wrap (&rest blocktypes)
    (cl-loop
     for type in blocktypes do
     (let ((newfunc (intern
  ...
  (defun diego|org-emphasize (&rest args)
...
(use-package smartparens
  ...
  (defun diego|sp-wrap-with (&rest args)
...
(defun diego|create-align-repeat-x (&rest args)
...
Source: https://git.io/vQKu4"
  (cl-loop
   ...
   (let ((new-func (intern (concat "diego/align-repeat-" name))))
     (eval `(defun ,new-func (start end switch)
              (interactive "r\nP")
...
*** Justify
#+begin_src emacs-lisp
(defun diego|justify (type &rest types)
