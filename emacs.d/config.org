...
#+begin_src emacs-lisp
(with-eval-after-load 'erc
  (defmacro d/make-erc-nick-format-func (name args nick cond &optional else)
    `(defun ,(intern name) ,args
       (if ,cond
           (let* ((nick ,nick)
  ...
  (d/make-erc-nick-format-func
   "d/erc-format-nick" (&optional user channel-data)
   (erc-server-user-nickname user)
   user)

  (d/make-erc-nick-format-func
   "d/erc-format-my-nick" nil
   (erc-current-nick)
   erc-show-my-nick
   (let ((prefix "> "))
     (erc-propertize prefix 'font-lock-face 'erc-default-face)))

  (advice-add #'erc-format-my-nick :override #'d/erc-format-my-nick)

  (defun d/erc-format-privmessage (nick msg privp msgp)
  ...
  (advice-add #'erc-format-privmessage :override #'d/erc-format-privmessage)

  (defun d/erc-track-find-face (fn faces)
    (if (erc-query-buffer-p)
        (setq ad-return-value (intern "erc-current-nick-face"))
      (funcall fn faces)))

  (advice-add #'erc-track-find-face :around #'d/erc-track-find-face)

  (defun d/erc-track-modified-channels (fn)
  ...
  (advice-add #'erc-track-modified-channels :around #'d/erc-track-modified-channels)

  (defun d/erc-notifications-notify (nick msg)
      ...
      (dbus-ignore-errors
        (setq erc-notifications-last-notification
              (notifications-notify :bus erc-notifications-bus
                                    :title (xml-escape-string channel)
                                    :body (xml-escape-string (concat nick ": " msg))
                                    :replaces-id erc-notifications-last-notification
                                    :app-icon erc-notifications-icon)))))

  (advice-add #'erc-notifications-notify :override #'d/erc-notifications-notify)
