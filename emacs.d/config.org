...
(define-globalized-minor-mode global-rlines-mode rlines-mode
  rlines-mode
  (lambda () (rlines-mode 1)))

(defmacro fbound-and-true-p (name &optional args)
  `(and (fboundp #',name)
        (apply #',name ,args)))
#+end_src
** Packages
...
#+begin_src emacs-lisp
(with-eval-after-load 'org
  (defmacro diego|create-block-wrap (&rest blocktypes)
    `(progn
       ,@(cl-loop
  ...
  (diego|create-block-wrap
   "src"
   "src python"
   "src emacs-lisp"
   "export latex")

  (defmacro diego|org-emphasize (&rest args)
    "Make functions for setting the emphasis in org mode"
    `(progn
       ,@(cl-loop for (name char) on args
                  by #'cddr collect
                  (let ((fname (intern (concat "diego/org-" name))))
                    `(defun ,fname ()
                       (interactive)
                       (org-emphasize ,char))))))
...
(with-eval-after-load 'org
  (diego/mode-leader-keys
   ...
   "xi" 'diego/org-italic
   "xc" 'diego/org-code
   "xu" 'diego/org-underline
   "xv" 'diego/org-verbatim
   "xs" 'diego/org-strike-through
   "xr" 'diego/org-clear
...
(use-package which-key
  ...
  (defmacro diego/declare-prefix (&rest body)
    (declare (indent defun))
    `(diego/leader-keys
      ,@(cl-loop for (prefix name) on body
                 by #'cddr
                 as j = `(:ignore t :which-key ,name)
                 while name
                 append `(,prefix ',j))))

  (defmacro diego/declare-map-prefix (maps &rest body)
    (declare (indent defun))
    `(diego/mode-leader-keys
      :keymaps ,maps
      ,@(cl-loop for (prefix name) on body
  ...
  (diego/declare-map-prefix 'org-mode-map
    "i" "insert"
    "x" "text"
    "t" "table"
    "e" "eval-export")

  (diego/declare-map-prefix '(emacs-lisp-mode-map lisp-interaction-mode-map)
    "e" "eval")
...
(mmap "RET"
      (general-predicate-dispatch nil
        (thing-at-point 'url) 'go-to-address-at-point
        (fbound-and-true-p org-at-openable-item-p) 'org-open-at-point
        (fbound-and-true-p org-at-item-checkbox-p) 'org-toggle-checkbox
        (fbound-and-true-p org-in-src-block-p) 'org-babel-execute-src-block))
#+end_src
*** Leader
#+begin_src emacs-lisp
(diego/leader-keys
...
(use-package define-word
  ...
  :general
  (diego/leader-keys "sw" 'diego/define-word)
  :config
  (defun diego/define-word (&optional word)
    (interactive)
    (if word
        (define-word word)
...
(use-package devdocs
  :general
  (diego/leader-keys "hdd"  'devdocs-search))
#+end_src
...
(use-package headlong
  :general
  (diego/leader-keys "fB" 'headlong-bookmark-jump-other
...
(use-package smartparens
  ...
  (defmacro diego|sp-wrap-with (&rest args)
...
(use-package undo-tree
  :general
  (diego/leader-keys "au" 'undo-tree-visualize)
...
(use-package unfill
  :general
  (diego/leader-keys "xq" 'unfill-toggle)
...
(use-package yasnippet
  :defer 5
  :general (diego/leader-keys "iy" 'yas-insert-snippet)
  :config
  (yas-global-mode 1))
#+end_src
...
(defmacro diego|create-align-repeat-x (&rest args)
...
*** Justify
#+begin_src emacs-lisp
(defmacro diego|justify (&rest types)
...
(defun diego/expand-url-at-point ()
  (interactive)
  (if (thing-at-point 'url)
...
(defun propertize-active (window-active &rest arguments)
...
(defmacro with-window-status (&rest body)
  (declare (indent defun))
  `(let ((window-active? (powerline-selected-window-active)))
     ,@body))

(defmacro with-evil-tag-color (&rest body)
  (declare (indent defun))
  `(let ((evil-tag-color
...
(defun diego/make-xpm (color height width)
...
(defun in-evil-substitution-p ()
  (or (assq 'evil-ex-substitute evil-ex-active-highlights-alist)
      (assq 'evil-ex-global-match evil-ex-active-highlights-alist)
      (assq 'evil-ex-buffer-match evil-ex-active-highlights-alist)))

(defun evil-substitute-num-matches ()
...
(defvar diego/mode-line
  '((:eval
     (with-evil-tag-color
       (with-window-status
         (cond ((and window-active?
                     (not (or (eyebrowse-relevant-p)
                              (bound-and-true-p anzu--state)
                              (in-macro-p)
                              (in-evil-substitution-p))))
                (diego/make-xpm evil-tag-color 56 8))
               ((not window-active?)
                (diego/make-xpm "#1D2021" 56 8))))))
    ((:eval
      (with-window-status
        (with-evil-tag-color
          (when window-active?
            (propertize-active
             window-active?
             (cond ((in-evil-substitution-p)
                    (evil-substitute-num-matches))
                   ((in-macro-p)
    ...
    ;; Line/column number
    (:eval (with-window-status
             (propertize-active window-active?
                                "%4l:%2c  "
...
(with-current-buffer "*Messages*"
  (setq-local mode-line-format diego/mode-line))
#+end_src
** Packages
...
(defun diego/pp-eval-surrounding-sexp (levels)
...
(defun diego/pp-macroexpand-surrounding-sexp (levels)
  (interactive "p")
  (let (pp-escape-newlines)
    (save-excursion
      (sp-end-of-sexp (abs levels))
...
(diego/mode-leader-keys
 ...
 "es" 'diego/eval-surrounding-sexp
 "el" 'eval-last-sexp
 "pm" 'diego/pp-macroexpand-surrounding-sexp
 "ps" 'diego/pp-eval-surrounding-sexp)
 #+end_src
*** Setup
#+begin_src emacs-lisp
...
(use-package sane-term
  :general (diego/leader-keys "at" 'sane-term-create))
#+end_src
...
(use-package restart-emacs
  :general (diego/leader-keys "qr" 'restart-emacs)
  :init
...
(use-package paradox
  :general
  (diego/leader-keys "ap" 'paradox-list-packages)
...
_i_ aggressive-indent:   %-3s`aggressive-indent-mode   _l_ rlines:              %-3s`rlines-mode   _F_ focus:                 %-3s`focus-mode
...
(use-package speed-type
  :general (diego/leader-keys "as" 'speed-type-text)
  :config
...
#+begin_src emacs-lisp
(use-package xkcd
  :general (diego/leader-keys "ax" 'xkcd)
  :general
