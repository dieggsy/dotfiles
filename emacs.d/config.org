...
(use-package which-key
  ...
  (defmacro diego/declare-mode-prefix (modes &rest body)
    (declare (indent defun))
    (let ((modes (if (listp modes) modes (list modes))))
      `(progn
         ,@(cl-loop
  ...
  (defmacro diego/which-key-remove-prefix (&rest body)
    (declare (indent defun))
    `(progn
  ...
  (diego/declare-mode-prefix org-mode
    "i" "insert"
    "x" "text"
    "t" "table"
    "e" "eval-export")

  (diego/declare-mode-prefix (emacs-lisp-mode lisp-interaction-mode)
    "e" "eval"
    "p" "pp")
