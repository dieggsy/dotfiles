...
(defalias 'd/window-active? #'powerline-selected-window-active)

(defun d/propertize-active (&rest arguments)
  (let ((str (car arguments)))
    (cond ((and (stringp str)
                (or (d/window-active?)
                    (member 'display arguments)))
           ;; (and (d/window-active?) (stringp str))
           (apply #'propertize arguments) )
          ((stringp str)
           (car arguments)))))

(defun d/evil-tag-color ()
...
(defvar d/mode-line
  '((:eval
     (let* ((evil-tag-color (d/evil-tag-color))
            (window-active (d/window-active?))
            (properties `(face
                          (:foreground
                           "#3E3D31"
                           :weight bold
            ...
            (text (cond ((not window-active)
                         (setq window-active t)
                        ...
                        (t
                         (when (display-graphic-p)
                           (setq properties
                                 `(display ,(d/make-xpm evil-tag-color 47 8))))
                         " "))))
       (apply #'d/propertize-active text properties)))
    ...
    ;; Major mode
    (:eval
     (unless (and (file-name-extension (buffer-name))
                  (eq major-mode (alist-get (buffer-name)
                                            auto-mode-alist
                                            nil
                                            nil
                                            #'string-match-p)))
...
(d/after 'erc
  (setq
   ...
   erc-insert-timestamp-function 'd/erc-always-insert-timestamp-left
   erc-nick "dieggsy"
...
#+end_src
**** Appearance
Mostly a hack to make ERC look like weechat
#+begin_src emacs-lisp :tangle no
(d/after 'erc
...
#+begin_src emacs-lisp
(d/after 'erc
  (defun d/erc-always-insert-timestamp-left (_string)
    (let* ((ct (current-time))
           (ts-left (erc-format-timestamp ct "%H:%M "))
           (day-left (erc-format-timestamp ct erc-timestamp-format-left)))
      (goto-char (point-min))
      (unless (string-equal day-left erc-timestamp-last-inserted-left)
        (erc-put-text-property 0 (length day-left) 'field 'erc-timestamp day-left)
        (insert day-left)
        (setq erc-timestamp-last-inserted-left day-left))
