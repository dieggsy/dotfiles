...
;;; -*- lexical-binding: t -*-
#+end_src
Then we add a commented call to ~package-initialize~ because ~package.el~ is
annoying like that.
...
(use-package exwm
  :demand t
  :if (not (or (string= (getenv "XDG_CURRENT_DESKTOP") "i3")
               (string= (getenv "XDG_CURRENT_DESKTOP") "kde")))
  :init
  (defmacro d/exec (cmd)
    `(lambda () (interactive) (call-process-shell-command ,cmd)))
  :general
  ("M-SPC" (d/exec "rofi -combi-modi window,run,drun -show combi -modi combi")
   "s-u" 'exwm-layout-toggle-fullscreen
   "<XF86MonBrightnessUp>"   (d/exec "xbacklight +2")
   "<XF86MonBrightnessDown>" (d/exec "xbacklight -2")
   "<XF86KbdBrightnessDown>" (d/exec "kb-light -")
   "<XF86KbdBrightnessUp>"   (d/exec "kb-light +")
   "<XF86AudioMute>" (d/exec "amixer -D pulse sset Master toggle")
   "<XF86AudioLowerVolume>" (d/exec "amixer -D pulse sset Master 2%-")
   "<XF86AudioRaiseVolume>" (d/exec "amixer -D pulse sset Master 2%+"))
  :config
  (require 'exwm-config)
  (exwm-config-default)
  (exwm-input-set-simulation-keys nil)
  (exwm-enable)
  (defun networkmanager-dmenu ()
    (interactive)
    (call-process-shell-command "networkmanager_dmenu"))
  (setq exwm-input-prefix-keys `(?\C-/
                                 ,(elt (kbd "M-SPC") 0)
                                 ?\s-u
                                 XF86MonBrightnessUp
  ...
  (cl-defmacro d/process-for-mode-line (name &rest body &key cmd repeat fmt init &allow-other-keys)
    (declare (indent defun))
    (let ((body (cl-loop for (key value) on body by #'cddr
                         if (and value (not (keywordp key)))
                         append `(,key ,value)
                         else if (not (keywordp key)) collect key))
          (shell-cmd (format "while sleep %d; do %s; done" repeat cmd))
          (filter-name (intern (concat name "-filter")))
          (var-name (intern (concat name "-output"))))
      `(progn
  ...
  (d/process-for-mode-line "d/ssid"
    :cmd "iwgetid -r || echo 'None'"
    :repeat 5)

  (d/process-for-mode-line "d/battery"
      ...
      (:eval
       (let* ((lhs d/mode-line)
              ...
              (rhs `((:eval
                      (d/with-window-status
                        (concat
                         (propertize-active (concat d/ssid-output "  ")
                                            'face '(:foreground "#B8bb26"))

                         (propertize-active (concat d/battery-output "  ")
