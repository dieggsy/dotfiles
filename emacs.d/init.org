    ...
    (let ((init-tangle-start-time (current-time)))
      (async-start
       (lambda ()
         ...
         ;; Selectively condition-case some blocks
         (defvar d/src-params)

         (define-advice org-babel-tangle-single-block
             (:before (block-counter &optional only-this-block))
           (setq d/src-params (nth 2 (org-babel-get-src-block-info))))

         (defun d/org-wrap-with-condition-case ()
           (let ((src-text (buffer-string)))
             (unless (or (string= (alist-get :cond-case d/src-params) "no")
                         (string-prefix-p "(use-package" src-text)
                         (string-prefix-p "(d/with-eval-after-load" src-text))
               (beginning-of-buffer)
               (insert
                "(condition-case-unless-debug err\n    (progn\n")
               (end-of-buffer)
               (insert ")\n  (error (display-warning 'init (format \"init.el: %s\" (error-message-string err)))))"))))

         (add-hook 'org-babel-tangle-body-hook #'d/org-wrap-with-condition-case)
...
(use-package dired
  ...
  (nmap :keymaps 'dired-mode-map
    ;; "q" 'dired-quit
    "h" 'dired-up-directory ;; 'diredp-up-directory-reuse-dir-buffer
    "l" 'dired-open-file ;; 'diredp-find-file-reuse-dir-buffer
    "j" 'dired-next-line
    ...
    "th" '(:command dired-omit-mode :wk "hidden")
    "td" '(:command dired-du-mode :wk "du")
    "tw" '(:command dired-toggle-read-only :wk "wdired"))
  ...
  (defun dired-quit ()
    (interactive)
    (let* ((prev-buffers (buffer-list))
           (nondired-pos (cl-position-if
                          (lambda (buf)
      ...
      (set-window-prev-buffers nil (nthcdr nondired-pos prev-buffers))
      (quit-window)))

  (defun dired-here (arg)
    (interactive "P")
    (if arg
        (dired default-directory)
      (dired-other-window default-directory)))

  (define-advice dired-up-directory (:around (fn &optional other-window) preserve-quit-restore)
    (let ((last-quit-restore (window-parameter nil 'quit-restore))
          (prev-buffers (window-prev-buffers)))
      (funcall fn other-window)
      (set-window-parameter nil 'quit-restore (append
                                               (cl-subseq last-quit-restore 0 3)
                                               `(,(current-buffer))))
      (set-window-prev-buffers nil prev-buffers)))

  (define-advice dired-open-file (:around (fn &optional other-window) preserve-quit-restore)
    (let ((last-quit-restore (window-parameter nil 'quit-restore))
          (prev-buffers (window-prev-buffers)))
      (funcall fn other-window)
      (set-window-parameter nil 'quit-restore (append
  ...
  (define-advice dired-find-alternate-file (:around (fn) preserve-quit-restore)
    (let ((last-quit-restore (window-parameter nil 'quit-restore))
          (prev-buffers (window-prev-buffers)))
      (funcall fn)
      (set-window-parameter nil 'quit-restore (append
      ...
      (let* ((pos (re-search-forward
        ...
        (when url
          (make-process
           ...
           :stderr (get-buffer-create "*nix-prefetch-git*")
           :filter
  ...
  (d/setup-mode eshell
    (imap :keymaps 'eshell-mode-map
      "C-p" (general-predicate-dispatch nil
              (eshell-point-within-input-p) 'eshell/cool-peco-ps)
      "C-r" 'eshell/cool-peco-history
      "C-l" 'eshell-clear-keystroke))
  :config
...
(d/with-eval-after-load 'proced
  (defvar proced-mode-map)
  (evil-make-overriding-map proced-mode-map 'normal))
