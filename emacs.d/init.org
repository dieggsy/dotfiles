...
(use-package nix-mode
  ...
  (defun d/nix-update-my-packages ()
    (interactive)
    (cl-flet ((d/nix-update-filter
               (process output)
               (let ((prog-mode-hook nil)
                     (output (format " %s " (string-trim output)))
                     (visited-p (get-file-buffer
                                 "~/dotfiles/nix-local/packages.nix"))
                 ...
                 (with-current-buffer
                   ...
                   (save-excursion
                     ...
                     (and (re-search-forward
                           (rx "[" (group (minimal-match (0+ anything))) "]") nil t)
                          (replace-match output nil nil nil 1))
                     (indent-region (point-min) (point-max)))
                   (save-buffer)
                   (unless visited-p
                     (kill-this-buffer))))))
      (let ((proc (start-process-shell-command
                   "d/nix-update-my-packages"
                   "test"
                   (concat "echo \"$("
                           "nix-env -qaP "
                           "| grep -P \"$(nix-env -q | tr '\r\n' '|' | sed 's/|$//; s/|/| /g;')\" "
                           ...
                           "| sed 's/^nixpkgs\\.//'"
                           ")\""))))
        (set-process-filter proc #'d/nix-update-filter)))
    (let ((nix-files
           (split-string
            (shell-command-to-string
...
(use-package eshell
  ...
  (d/setup-hook eshell-mode
    ...
    (setq-local company-backends '(company-eshell)))
...
(use-package pcmpl-git
  :after eshell)
#+end_src
**** [[https://github.com/thierryvolpiatto/pcomplete-extension][pcomplete-extension]]
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(d/with-eval-after-load 'eshell
  (defun company-eshell-candidates (prefix)
    (let* ((history
            (cl-remove-duplicates
             (mapcar (lambda (str)
                       (string-trim (substring-no-properties str)))
  ...
  (defun company-eshell (command &optional arg &rest ignored)
    ...
    (cl-case command
      ...
      (candidates (company-eshell-candidates arg)))))
#+end_src
** Term
#+begin_src emacs-lisp
(use-package term
