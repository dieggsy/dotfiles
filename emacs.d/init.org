...
(use-package general
  ...
  ;;   (put (intern "general-" (symbol-name func)) 'lisp-indent-function 'defun))
...
(use-package nix-mode
  ...
  (defmacro d/nix-update-helper (search get-url)
    `(save-excursion
       ...
       (d/let lop ((bound
                    (re-search-forward
                     (rx "=" (opt " ") ,search (opt " ") "{" (group (minimal-match (0+ anything))) "}")
                     nil
                     'noerror)))
         (if (not bound)
             '()
               ...
               (github-urls
                (d/nix-update-helper
                 ...
                 (concat
                  "https://github.com/"
                  (and
                   (save-excursion (re-search-forward "\"?owner\"?[ ]*?=[ ]*?\"\\(.*\\)\"" bound)
                                   (match-string-no-properties 1)))
                  "/"
                  (and
                   (save-excursion (re-search-forward "\"?repo\"?[ ]*?=[ ]*?\"\\(.*\\)\"" bound)
                                   (match-string-no-properties 1))))))
                  ...
                  (let* ((json-vals (json-read-from-string output))
                    ...
                    (let ((visited-p (get-file-buffer ,file))
                      ...
                      (with-current-buffer file-buffer
                        ...
                        (save-excursion
                          (unless (string= (apply #'buffer-substring-no-properties
                                                  ',rev-bounds)
                                           rev)
  ...
  (defun d/nix-update-my-packages ()
    (interactive)
    (make-process
     :name "d/nix-update-my-packages"
     :command '("nix-env" "-q")
     :connection-type 'pipe
     :filter
     (lambda (process output)
       (let ((prog-mode-hook nil)
             (output (format " %s " (string-trim output)))
             (buffer-open (get-file-buffer "~/dotfiles/nix-local/packages.nix"))
             (inhibit-modification-hooks t))
         (with-current-buffer
             (find-file-noselect "~/dotfiles/nix-local/packages.nix")
           (save-excursion
             (goto-char (point-min))
             (and (re-search-forward (rx "[" (group (minimal-match (0+ anything))) "]") nil t)
                  (replace-match output ;; (format " %s " (string-trim output))
                                 nil nil nil 1))
             (goto-char (point-min))
             (while (re-search-forward "\\(-[.0-9-]+\\(-dev\\)?\\)\\_>" nil t)
               (replace-match "" nil nil nil 1))
             (indent-region (point-min) (point-max)))
           (save-buffer)
           (unless buffer-open
             (kill-this-buffer))))))
    (let ((nix-files (split-string (shell-command-to-string "grep -lrP 'fetchgit|fetchFromGitHub' ~/dotfiles/nix-local")
                                   "\n"
                                   'omit-nulls)))
