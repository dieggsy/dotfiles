...
(eval-when-compile
  (require 'use-package))
#+end_src
**** custom-after keyword
The ~:custom~ keyword is handy for using the customize interface for setting
variables, but I'd like to customize at config time to avoid copying over long
lists.
#+begin_src emacs-lisp
(defun use-package-normalize/:custom-after (name keyword args)
...
(defun use-package-handler/:custom-after (name keyword args rest state)
...
(defun use-package-normalize/:custom-face-after (name-symbol keyword arg)
...
(defun use-package-handler/:custom-face-after (name keyword args rest state)
...
(setq use-package-keywords
      (let* ((pos (cl-position :config use-package-keywords))
             (head (cl-subseq use-package-keywords 0 (1+ pos)))
             (tail (cl-subseq use-package-keywords (1+ pos))))
        (append head (list :custom-after :custom-face-after) tail)))
#+end_src
* Preface
** Packages
...
(use-package smartparens
  ...
  :custom
  (sp-ignore-modes-list '(scheme-mode
                          emacs-lisp-mode
                          lisp-mode))
  :config
...
(use-package calc
  :ensure nil
  :hook (prog-mode . d/calc-embedded-set-open-close)
  :general
  (d/leader-keys
   "ac" 'calc-dispatch)
  (emap calc-mode-map
