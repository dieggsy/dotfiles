...
#+end_src
** Environment
This is only relevant for starting GUI Emacs on macOS
#+begin_src emacs-lisp
(defun set-environment-from-shell ()
...
specified in `shell-file-name'."
  ...
  (let* ((shell (or shell-file-name (getenv "SHELL")))
         (proc
          (apply #'start-process
                 "printenv"
                 (get-buffer-create " *printenv*")
                 shell-file-name
                 (append
                  (cond ((string-match-p "t?csh$" shell)
                         (list "-d"))
     ...
     (lambda (proc msg)
       (when (eq (process-status proc) 'exit)
         (if (zerop (process-exit-status proc))
             (progn
               (with-current-buffer (get-buffer " *printenv*")
                 (let ((str (buffer-string)))
                   (dolist (var-pair
                     ...
                     (let ((var (car var-pair))
                           (value (cadr var-pair)))
                       (if (member var '("PATH" "INFOPATH" "MANPATH"))
                           (setenv var (concat value ":" (getenv var)))
                         (setenv var value))))))
               (dolist (dir (split-string (getenv "PATH") ":"))
                 (unless (member dir exec-path)
                   (add-to-list 'exec-path dir))))
           (warn "Error setting environment from shell.")))))))

(when (eq system-type 'darwin) (set-environment-from-shell))
#+end_src
** pass
...
(define-advice customize-push-and-save (:after (vars unsafe-vars) d/safe-vars)
  (when (eq vars 'safe-local-variable-values)
    (with-current-buffer (find-file-noselect (no-littering-expand-etc-file-name "safe-vars.el"))
      (erase-buffer)
      (insert (format "(setq safe-local-variable-values '%S)\n" safe-local-variable-values))
      (save-buffer))))

(let ((safe-vars (no-littering-expand-etc-file-name "safe-vars.el")))
  (when (file-exists-p safe-vars)
    (load safe-vars)))
#+end_src
*** Kill-ring
...
(use-package darktooth-theme
  ...
  :init
  (define-advice autothemer-deftheme (:before (_ _ palette &rest _) d/defcolors)
    (mapcar (lambda (e)
  ...
  (d/list-prepend! 'ivy-ignore-buffers
    ...
    (rx "*"
        (or "LV" "epc" "Calc" "Colors" "help" "Packages" "Customize" "info"
            ...
            "geiser messages*" "Geiser dbg" "tramp/" "Process List" "eshell:"
            "Error" "magit-todos" "clang" "lsp" "Flymake" "sly-inferior"
            "sly-events" "sly-compilation" "Async-native-compile-log"
...
color is hidden by ivy's match color."
  ...
  (ivy-configure 'ivy-switch-buffer :display-transformer-fn 'd/ivy-dired-transformer)
...
so this function ensures lexicographic order."
  ...
  (defun d/unicode (arg)
    (interactive (list (completing-read "unicode: " (d/unicode-names))))
    (insert (car (last (split-string arg))))))
...
(use-package consult
  ...
  (if (eq system-type 'darwin)
      (setf consult-locate-args "mdfind -name")
    (when (executable-find "plocate")
      (setf consult-locate-args "plocate --ignore-case --existing --regexp")))
  (setq consult-project-root-function
        (lambda ()
          (when-let (project (project-current))
            (car (project-roots project)))))
  (d/list-prepend! 'consult-buffer-filter
    (rx "*"
        (or "LV" "epc" "Calc" "Colors" "help" "Packages" "Customize" "info"
            "Compile" "anaconda-mode" "scratch" "Messages" "elfeed-log" "Man"
            "Quail" "Paradox Report" "Backtrace" "slime-events"
...
(use-package embark
  ...
  (selectrum-minibuffer-map
   "<S-return>" 'embark-act)
  :config
  (setq embark-which-key-indicator-excluded-actions '(digit-argument negative-argument mark delete-region))
    ...
    (lambda (&optional keymap targets prefix)
      (if (null keymap)
        ...
        (let ((which-key-replacement-alist
               (append '(((nil . "digit-argument") . t)
                         ((nil . "delete-region") . t)
                         ((nil . "mark") . t)
                         ((nil . "negative-argument") . t))
...
(add-hook 'pre-redisplay-functions
          (defun d/set-selected-window (_)
            (unless (eq (selected-window) (minibuffer-window))
              (setq d/selected-window (selected-window)))))
(defun d/window-active-p ()
  (eq (selected-window) d/selected-window))

(cl-defun propertize-active (thing &optional (active-face 'mode-line-active) (inactive-face 'mode-line-inactive))
  (propertize thing
              'face
              ...
              '("%e"
                (:eval
                 (propertize-active
                  ...
                  (pcase evil-state
                    (normal   '(:foreground "#282828" :background "#B8BB26"))
                    (insert   '(:foreground "#282828" :background "#66999D"))
                    (motion   '(:foreground "#282828" :background "#D3869B"))
                    (visual   '(:foreground "#282828" :background "#FE8019"))
                    (emacs    '(:foreground "#282828" :background "#FABD2F"))
                    (replace  '(:foreground "#282828" :background "#FE8019"))
                    (operator '(:foreground "#282828" :background "#B8BB26")))
                  '(:foreground "#282828" :background "#665C54")))
                " "
                (:eval
                 (concat (let ((bufname (format-mode-line "%b" t)))
                           (if (string-match "^\\*\\(.*\\)\\*\\(<[0-9]+>\\)?$" bufname)
                               (propertize (concat (match-string 1 bufname)
                                                   (match-string 2 bufname))
                                           'face '(:slant italic))
                             (propertize-active bufname t 'mode-line-inactive)))))
                " "
                (:eval
                 (let ((host (file-remote-p default-directory 'host)))
                   (when host
                     (propertize-active
                      (concat "@" host  " ")
                      '(:inherit ivy-remote :slant italic)
                      '(:inherit mode-line-inactive :slant italic)))))
                (:eval
                 (concat (propertize-active "(" '(:foreground "#5C7E81"))
                         (propertize-active
                          (if (listp mode-name) (car mode-name) mode-name)
                          '(:inherit mode-line-active :slant italic)
                          '(:inherit mode-line-inactive :slant italic))
                         (if vc-mode
                         ...
                         (propertize-active ")" '(:foreground "#5C7E81"))))
                " "
                (:eval
                 (propertize-active
...
(use-package dired
  ...
  (dired-dwim-target t)
  :config
  (setq dired-listing-switches (if (member system-type '(darwin berkeley-unix))
                                   "-lhA"
                                 "-lXhA --group-directories-first"))
  (d/hook! dired-mode
    (auto-revert-mode))
  (nmap dired-mode-map
    "~" 'd/dired-home
...
**** undo-fu
#+begin_src emacs-lisp
(use-package undo-fu :straight (:protocol https))
#+end_src
...
#+end_src
*** [[elisp:(find-library-other-window "browse-url")][browse-url]]
#+begin_src emacs-lisp :tangle no
...
(use-package xref
  ...
  ;;           (defun d/run-project-ctags ()
  ;;             (interactive)
  ;;             (let ((root-dir (vc-root-dir)))
  ;;               (when root-dir
  ;;                 (let ((default-directory root-dir))
  ;;                   (unless (get-process "d/ctags")
  ;;                     (start-process "d/ctags" "*ctags*" "ctags" "-eR")))))))
  )
#+end_src
** Packages
...
*** Font
#+begin_src emacs-lisp
(add-to-list 'default-frame-alist '(font . "Iosevka Term-11:weight=book"))
(defun d/setup-fonts (&rest ignore)
  (setq inhibit-compacting-font-caches t
        use-default-font-for-symbols nil)
  ;; Emoji
  (set-fontset-font t '(#x1F600 . #x1F64F) "Noto Color Emoji" nil 'prepend)
  ;; Music
...
(use-package whitespace
  :defer 5
  :config
  (setq whitespace-style '(face tabs trailing empty tab-mark missing-newline-at-eof))
...
(use-package python
  ...
  (let ((ipython (executable-find "ipython")))
    (setq python-shell-interpreter (or ipython
                                       python-shell-interpreter)
          python-shell-interpreter-args (if ipython "--simple-prompt"
...
(use-package which-key
  ...
  :after embark
  :config
  (setq which-key-inhibit-regexps '("[:digit:]" "-"))
