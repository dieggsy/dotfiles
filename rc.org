...
super + w
    rofi -show window -location 1 -yoffset 62 -xoffset 15 -theme-str "#window \{width: 800;\}"

super + d
    rofi -show drun -location 1 -yoffset 62 -xoffset 15 -show-icons
...
alt + space
    rofi -show run -location 1 -yoffset 62 -xoffset 15

# passwords
super + p
    passmenu -location 1 -yoffset 62 -xoffset 15 -p "pass"
...
[dmenu]
# l = 10
dmenu_command = rofi -location 1 -yoffset 62 -xoffset 250 -no-fixed-num-lines -theme-str "* {highlightcolor: #83A598;}" -theme-str "#window {width: 330;}" -theme-str "#inputbar {enabled:false;}"
rofi_highlight = True
...
# rofi -location 1 -dmenu -i -theme-str "#window {width: 500;}"
...
inp = "\n".join(flattened)
print(Popen(ROFI_CMD + [ "-a", str(active) ], stdin=PIPE, stdout=PIPE).communicate(input=inp.encode(ENC))[0].decode(ENC).strip())
#+end_src
** mons-rofi
...
response=$(echo -e "exit\nlock\nsuspend\nhibernate\nreboot\nshutdown" |
               rofi -dmenu -location 1 -yoffset 62 -xoffset 15)
if [ -n "$response" ]; then
