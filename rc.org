...
bspc rule -a .emacs-wrapped state=tiled
...
(with-output-to-file (create-temporary-file)
  (lambda ()
    (display title)))

(cond ((equal? instance "-emacs-wrapped")
       (cond ((equal? title "*eshell: floating*")
             ...
             ((equal? title "*Minibuf-0*")
              (with-output-to-file (create-temporary-file)
                (lambda ()
                  (display "YES")))
              (display ("state=floating sticky=on"))))))
#+end_src
*** sxhkdrc
:PROPERTIES:
...
# terminal emulator
super + Return
    id=$(cat /tmp/esh-float); bspc node $id --flag hidden; bspc node -f $id

super + space
    id=$(cat /tmp/erc-float); bspc node $id --flag hidden; bspc node -f $id
...
modules-right = dropbox spotify cmus inbox-reddit github weather filesystem xbacklight volume battery

wm-restack = bspwm

cursor-click = pointer
cursor-scroll = ns-resize
click-left = BLOCK_BUTTON=1 ~/bin/i3blocks/layout
enable-ipc = true
...
[module/filesystem]
type = custom/script
exec = df -h / | grep -v "^[A-Z]" | awk '{print $4}'
interval = 25
...
type = custom/script
exec = ~/bin/i3blocks/chicken-wifi
...
[module/weather]
type = custom/script
exec = ~/bin/i3blocks/chicken-weather
...
[module/cmus]
type = custom/script
exec = ~/bin/i3blocks/chicken-music
tail = true
...
[module/spotify]
type = custom/script
exec = ~/bin/i3blocks/chicken-music spotify
tail = true
format-prefix = " "
format-prefix-foreground = ${colors.green}
...
[module/volume]
type = custom/ipc
hook-0 = ~/bin/i3blocks/volume
initial = 1
scroll-down = ~/bin/i3blocks/volume + && polybar-msg -p %pid% hook volume 1
scroll-up = ~/bin/i3blocks/volume - && polybar-msg -p %pid% hook volume 1
click-left = ~/bin/i3blocks/volume toggle && polybar-msg -p %pid% hook volume 1
