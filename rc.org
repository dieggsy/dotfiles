...
(cond ((substring-index "emacs" instance)
      ...
      ((substring-index "_erc__floating_" instance)
       ...
       (display "state=floating hidden=on sticky=on rectangle=1085x560+0+870"))
      ...
      ((equal? instance "st-256color")
       ...
       (when (equal? title "st-float")
         (with-output-to-file "/tmp/st-float"
           (lambda () (display wid) (newline)))
         (display "state=floating hidden=on sticky=on rectangle=1085x560+1465+870")))
      ((and (equal? instance "guvcview")
...
setopt prompt_subst
export PROMPT='$(maybe_host)$(maybe_git)%F{7}%1~%f %F{167}%(!.#.λ)%f '
