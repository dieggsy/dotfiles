...
# password-store helper with rofi
bindsym $mod+p exec "~/.password-store/.extensions/rofi.bash"

# clipboard manager helper with rofi
...
# File finder with ripgrep and rofi
bindsym $mod+f exec --no-startup-id "~/.zplug/bin/rg --files --no-ignore --hidden -g '!{.git,node_modules,.cache}/*'  2>/dev/null | rofi -dmenu -i -levenshtein-sort -matching regex -font 'Iosevka 20' -p '[files] ' | xargs xdg-open"
...
tar -xf ~/dotfiles/installed-programs/apt.tgz /tmp/apt
...
mkdir -p /tmp/apt
dpkg --get-selections > /tmp/apt/package.list
sudo cp -R /etc/apt/sources.list* /tmp/apt/
sudo apt-key exportall > /tmp/apt/repo.keys
cd /tmp/ && tar cf ~/dotfiles/installed-programs/apt.tgz apt/ && cd -
#+end_src
*** Finalize
#+begin_src sh
echo " "
