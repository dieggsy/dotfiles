...
for_window [instance="metask"] move scratchpad; [instance="metask"] scratchpad show; resize set 2446 607; move position center; move up 420px; move scratchpad
...
# some controls
bindsym XF86KbdBrightnessDown exec kb-light - && pkill -RTMIN+1 i3blocks
bindsym XF86KbdBrightnessUp exec kb-light + && pkill -RTMIN+1 i3blocks

bindsym XF86MonBrightnessUp exec xbacklight +2 && pkill -RTMIN+1 i3blocks
bindsym XF86MonBrightnessDown exec xbacklight -2 && pkill -RTMIN+1 i3blocks
...
*** i3blocks
:PROPERTIES:
:header-args+: (when (eq system-type 'gnu/linux "~/.i3blocks.conf"))
:END:
#+begin_src conf
[focused window]
command=xtitle -s
...
[Brightness]
command=~/.config/i3blocks/scripts/bright.sh
interval=once
signal=1
...
[layout]
command=~/.config/i3blocks/scripts/layout.sh
interval=once
...
*** vol
:PROPERTIES:
:header-args+: :tangle (when (eq system-type 'gnu/linux) "~/.config/i3blocks/scripts/volume") :shebang "#!/usr/bin/bash"
:END:
#+begin_src sh
