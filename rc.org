...
bindsym $mod+f exec --no-startup-id "~/.zplug/bin/rg --files --hidden -g '!{.git,node_modules,.cache}/*'  2>/dev/null | rofi -dmenu -i -levenshtein-sort -matching regex -font 'Iosevka 20' -p '[files] ' | xargs -d '\n' xdg-open"
#+end_src
