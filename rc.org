...
bindsym Mod1+space exec --no-startup-id "rofi -combi-modi window,run,drun -show combi -modi combi"
...
:END:
#+begin_src sh
feh --bg-fill '/home/diego/Pictures/darkgray2.png' &
...
pkill xautolock; xautolock -time 5 -locker '/usr/local/bin/lock -gpf Iosevka-Term -- scrot -z' -detectsleep &
...
if [ "$mode" = "Backlight" ]; then
    ...
    if [ "$XDG_CURRENT_DESKTOP" = "exwm" ] && [ "$2" = "+" ];then
        brightness=$((($brightness + 5)))
        xbacklight -set $brightness > /dev/null 2>&1
    elif [ "$XDG_CURRENT_DESKTOP" = "exwm" ] && [ "$2" = "-" ];then
        brightness=$((($brightness - 5)))
        xbacklight -set $brightness > /dev/null 2>&1
...
if [ "$XDG_CURRENT_DESKTOP" = "exwm" ] && [ "$Mute" = "on" ]; then
    emacsclient --eval "(message \"Volume: Mute\")"
