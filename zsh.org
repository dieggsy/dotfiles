...
PROMPT='${$(git_status)}%F{7}$(prompt_sorin_pwd)%f%b '
#+end_src
*** Cursor
...
**** cool-peco
#+begin_src sh
zplug "therockmandolinist/cool-peco"
#+end_src
**** rg
#+begin_src sh
...
alias sbcl="rlwrap -irc -pGreen -S 'SBCL> ' sbcl"
...
edit () {
    emacsclient -n $@
    (( $+commands[i3-msg] )) && i3-msg '[title="emacs@Ragnarok"] scratchpad show' > /dev/null 2>&1
}
